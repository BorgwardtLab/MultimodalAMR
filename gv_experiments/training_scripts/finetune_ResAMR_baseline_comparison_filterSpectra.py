...
def main(args):
    ...
    pretrained_model_folder = join(pretrained_checkpoints_folder, "{}_{}".format(target_species, target_drug), "pretraining", f"split_{split_idx}")
    ...
    data_folder = join(output_folder, "finetuning_data_splits", f"split_{split_idx}")
    ...
    if not os.path.exists(checkpoints_folder):
        raise FileNotFoundError(f"Pretrained model folder {checkpoints_folder} not found")
    print("Checkpoint folder ", checkpoints_folder)
    checkpoint_fname = os.listdir(checkpoints_folder)[0]
    print("Checkpoints file ", checkpoint_fname)
    checkpoint_path = join(checkpoints_folder, checkpoint_fname)
