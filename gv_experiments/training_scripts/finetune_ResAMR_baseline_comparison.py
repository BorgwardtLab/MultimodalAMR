...
def main(args):
    ...
    print(f"Combination {comb_idx} - Split {split_idx} - Target species {target_species} - Target drug {target_drug}")
    
    pretrained_checkpoints_folder = config["pretrained_checkpoints_folder"]
    # pretrained_model_folder = join("outputs", args.experiment_group, args.experiment_name)
    
    output_folder = join("outputs", args.experiment_group, args.experiment_name, "finetuning", "{}_{}".format(target_species, target_drug), f"split_{split_idx}")
    experiment_folder =  join("outputs", args.experiment_group, args.experiment_name, "finetuning", "{}_{}".format(target_species, target_drug))
    ...
    # checkpoints_folder = join(pretrained_model_folder, f"{seed}", "checkpoints", "{}_{}".format(target_species.replace(" ", "_"), target_drug.replace(" ", "_")))
    model_checkpoint_folder = join(pretrained_checkpoints_folder, "{}_{}".format(target_species.replace(" ", "_"), target_drug.replace(" ", "_")))
    if not os.path.exists(model_checkpoint_folder):
        raise FileNotFoundError(f"Pretrained model folder {model_checkpoint_folder} not found")
    print("Checkpoint folder ", model_checkpoint_folder)
    checkpoint_fname = os.listdir(model_checkpoint_folder)[0]
    if "val_loss=0.000" in checkpoint_fname:
        raise ValueError("Invalid checkpoint with null loss")
    ...
    trainval_df = target_combination_df[~target_combination_df["sample_id"].isin(test_split)]
    
    if config["filter_hh"]=="true" or config["filter_hh"]=="True":
        with open(config["workstations_mapping"], "r") as f:
            workstations_mapping = json.load(f)
        hh_samples = []
        for sid, ws in workstations_mapping.items():
            if ws=="HospitalHygiene":
                hh_samples.append(sid)
        hh_samples = set(hh_samples)
        n_tv_samples = len(trainval_df)
        trainval_df = trainval_df[~trainval_df["sample_id"].isin(hh_samples)]
        print("Filtered {} HospitalHygene samples".format(n_tv_samples - len(trainval_df)))
    ...
    if "val_loss=0.000" in checkpoint_callback.best_model_path:
        ...
        raise RuntimeError("Pytorch Lightning bugged out, please rerun the script")
        sys.exit(1)
    ...
    precision, recall, thresholds = precision_recall_curve(test_df["response"], test_df["predicted_proba"])
...
if __name__=="__main__":
    ...
    parser.add_argument("--workstations_mapping", type=str, default="")
    # parser.add_argument("--filter_hh", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--filter_hh", type=str, default="false")
