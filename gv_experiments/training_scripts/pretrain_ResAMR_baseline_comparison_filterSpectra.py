...
def main(args):
    ...
    # comb_idx = config["combination_idx"] #args.seed//5
    ...
    output_folder = join("outputs", args.experiment_group, args.experiment_name, "{}_{}".format(target_species, target_drug), "pretraining", f"split_{split_idx}")

    root_folder = config.get("root_folder", None)
    if root_folder is not None:
        output_folder = join(root_folder, output_folder)
    experiment_folder = output_folder
    # metrics_folder = join("outputs", args.experiment_group, args.experiment_name, "pretraining_metrics")
    ...
    test_samples = target_sets[split_idx]
    # for split in target_sets:
    #     test_samples.extend(split)
    ...
    print("Removed {} samples".format(n_tv_samples_prefiltering-len(trainval_df)))
    ...
    test_df.to_csv(join(predictions_folder, f"test_pretrained_{target_species.replace(' ', '_')}_{target_drug.replace(' ', '_')}_split{split_idx}.csv"), index=False)
...
if __name__=="__main__":
    ...
    parser.add_argument("--experiment_name", type=str, default="ResAMR_Pretraining")
    parser.add_argument("--experiment_group", type=str, default="PaperComparison_SeparatePretraining")
    ...
    parser.add_argument("--root_folder", type=str,
                        default="/home/gvisona/Projects/AMR_Pred/gv_experiments")
