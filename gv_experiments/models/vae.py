...
class BetaVAE(nn.Module):
    def __init__(self, config): #input_size=512, embedding_dim=32, beta=1.0, gamma=1000):
        ...
        for k in range(len(hidden_layer_sizes)-1):
            encoder_layers.append(nn.Linear(hidden_layer_sizes[k], hidden_layer_sizes[k+1]))
        ...
        # decoder
        decoder_layer_sizes = [self.embedding_dim] + [32, 64, 64, 64, 128, 256, 256] + [self.input_size] #hidden_layer_sizes[::-1]
        decoder_layers = []
        for k in range(len(decoder_layer_sizes)-1):
            decoder_layers.append(nn.Linear(decoder_layer_sizes[k], decoder_layer_sizes[k+1]))
