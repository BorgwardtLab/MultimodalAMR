...
import os
print(os.environ['longTable'])
longTable=os.environ['longTable']
print(os.environ['driam'])
driam=os.environ['driam']
print(os.environ['drug'])
drug=os.environ['drug']
print(os.environ['species'])
...
class DataSplitter():
    ...
    def __init__(self, long_table=None, dataset=None):
        if dataset is not None:
            self.long_table = long_table[long_table["dataset"]==dataset]
    ...
    def baseline_selection(self, drug=None, species=None):
        ...
        assert sum(ix)>0, f"No split available for combination drug {drug} - species {species}"
    ...
    def combination_train_test_split(self, df, test_size=0.2, random_state=42):
        ...
        species = sorted(list(df["species"].unique()), key=lambda x: species_counts_mapping[x])
        ...
        for sp in species:
            ...
            sel_samples = species_pairings.sample(weights=(1/species_pairings["species_counts"]))
            if train_pairings is None:
                train_pairings = sel_samples
            else:
                train_pairings = pd.concat((train_pairings, sel_samples))
        
        df3 = df2[~df2[["drug", "species"]].apply(tuple, 1).isin(train_pairings[["drug", "species"]].apply(tuple, 1))]
        ...
        for dr in drugs:
            ...
            sel_samples = drug_pairings.sample(weights=(1/drug_pairings["drug_counts"]))
            train_pairings = pd.concat((train_pairings, sel_samples))

        # Fill in test set
        df3 = df2[~df2[["drug", "species"]].apply(tuple, 1).isin(train_pairings[["drug", "species"]].apply(tuple, 1))].reset_index(drop=True)
        n_test = int(test_size*len(df))# - train_pairings['combination_counts'].sum()
        test_combination_ixs = sum_solver(df3["combination_counts"].values, n_test)
        test_pairings = df3.iloc[test_combination_ixs]
        train_data = df[~df[["drug", "species"]].apply(tuple, 1).isin(test_pairings[["drug", "species"]].apply(tuple, 1))].reset_index(drop=True)
        test_data = df[df[["drug", "species"]].apply(tuple, 1).isin(test_pairings[["drug", "species"]].apply(tuple, 1))].reset_index(drop=True)
...
#Data splitting
data = pd.read_csv("{}".format(longTable))
dsplit = DataSplitter(data, dataset="{}".format(driam))
     
#Drug zero-shot split
target_drug_df, train_val_df = dsplit.drug_zero_shot_split(drug="{}".format(drug))
target_drug_df
    
#Export
target_drug_df.to_csv(path+'test_data_zero_shot.csv')
...
train_data.to_csv(path+'train_data_zero_shot.csv')
val_data.to_csv(path+'val_data_zero_shot.csv')
